...
func GetKeyPair(file string) ([]byte, []byte, error) {
	// read keys from file
	_, err := os.Stat(file)
	if err == nil {
		...
		if err != nil {
			log.Println("Failed to read pub file -", err)
			goto genKeys
		}
		return pub, priv, nil
	}
...
genKeys:
	...
	if err != nil {
		return nil, nil, fmt.Errorf("Failed to write file - %s", err)
	}
	err = ioutil.WriteFile(file+".pub", []byte(pub), 0644)
	if err != nil {
		return nil, nil, fmt.Errorf("Failed to write pub file - %s", err)
	}
...
func GenKeyPair() ([]byte, []byte, error) {
	...
	return public, private.Bytes(), nil
}
